...
func (p *GrpcPrinter) Print(b *Build) error {
	...
	filename := strcase.ToSnake(string(service.Name))
	filename = fmt.Sprintf("%s.pb.go", filename)
...
func (p *GrpcEnumPrinter) Print(b *Build) error {
	if p.ImportPath == "" {
		// TODO: we don't support multiple package currently..
		// So p.ImportPath must be given.
		// p.ImportPath = string(p.File.GoImportPath)
	}

	import_path := strings.Split(p.ImportPath, "/")
	file_path := filepath.Join(p.ImportPath, "enum.pb.go")
	o := p.Plugin.NewGeneratedFile(file_path, protogen.GoImportPath(p.ImportPath))
	o.Write([]byte(`// Code generated by "protoc-gen-entpb". DO NOT EDIT`))
	o.Write([]byte(fmt.Sprintf("\n\npackage %s\n\n", import_path[len(import_path)-1])))
	...
	for _, proto_file := range p.Plugin.Files {
		known_imports.Pb = proto_file.GoImportPath
		for _, pb_enum := range proto_file.Enums {
			var ent_enum *enum
